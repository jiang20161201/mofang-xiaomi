$(function() {
    $(".toptoptop").animate({ "marginTop": 0, "opacity": 1 }, 2000);
...
$(function() {
    ...
    for (var i = 0; i < banner_lis.length; i++) {
        ...
        banner_lis[i].onmouseover = function() { //给li绑定鼠标经过事件
            for (var i = 0; i < banner_lis.length; i++) { //遍历
                ...
                animate(banner_div[i], { "left": "215", "opacity": 0 });
            }
            banner_div[this.index].style.display = "block";
            animate(banner_div[this.index], { "left": "235", "opacity": 0.8 });
    ...
    var timeId = null;
    timeId = setInterval(rightHandle, 3000);
    //设置鼠标进入box后显示显示左右焦点层
